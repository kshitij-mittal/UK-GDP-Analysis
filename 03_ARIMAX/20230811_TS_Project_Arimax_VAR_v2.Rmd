...
```{r}
par(mfrow = c(4, 1))
autoplot(ts_ipi, main = "IPI - Industrial Production Index -  Time Series")
autoplot(log(ts_ipi), main = "Log IPI Time Series")
autoplot(ts_int, main = "Interest Rates and Asset Prices Time Series")
autoplot(ts_cap, main = "Labour Capital Productivity Time Series")
par(mfrow = c(1, 1))
```
...
## Model 1 - Non Seasonal Auto ARIMAX with IPI Regressor
```{r}
arimax_model <- auto.arima(ts_gdp_train, xreg =window(ts_ipi, end = c(2010, 4)), seasonal = FALSE, lambda = bc_lambda, trace = TRUE)
# arimax_model <- Arima(ts_gdp_train, order = c(3,1,2) ,xreg = window(ts_ipi, end = c(2010, 4)), lambda = bc_lambda, include.drift = TRUE)
...
## Model 1-2 - Non Seasonal Auto ARIMAX with Log IPI Regressor
```{r}
arimax_model_log <- auto.arima(ts_gdp_train, xreg =window(log(ts_ipi), end = c(2010, 4)), seasonal = FALSE, lambda = bc_lambda, trace = TRUE)
arimax_model_log <- Arima(ts_gdp_train, order=c(2,2,2), xreg =window(log(ts_ipi), end = c(2010, 4)), lambda = bc_lambda, include.drift = TRUE )

arimax_model_log
coeftest(arimax_model_log)
```

```{r}
# Forecast using the ARIMAX model
forecast_arimax_log <- forecast(arimax_model_log, xreg = window(log(ts_ipi), start = c(2011, 1)), h = number_of_forecasts)

# Plot the forecast results
plot(forecast_arimax_log)

# Checking for RMSE
squared_diff_arimax_log <- (forecast_arimax_log$mean - ts_gdp_test)^2
rmse_arimax_log <- sqrt(mean(squared_diff_arimax_log))

# Calculate accuracy metrics (e.g., MAE, RMSE) to evaluate the forecast
accuracy(forecast_arimax_log, ts_gdp_test)

# Check residuals
checkresiduals(residuals(forecast_arimax_log))

# Print RMSE
print(paste("RMSE:", rmse_arimax_log))
```
...
summary(arimax_model_exog)
coeftest(arimax_model_exog)
```
...
accuracy(exp(pred$fcst$y1[, "fcst"]), ts_gdp_test)
