...
```{r}
kpss.test(ts_gdp_train_seasonal_diff)

```
P-value 0.1 > 0.05 ==> fail to reject the null hypothesis (the process is stationary). So, seasonal differencing helped to get stationary data. 
...
Model_sarima
# RUNNING EACF ON FIRST ORDER DIFFERENCE TRAIN DATA
eacf(ts_gdp_train_seasonal_diff)
```


EACF run for seasonal differenced data indicates using (0,3) and (1,3) models. We can run these also to check if we observe better model fits


```{r}
fit_sar1=Arima(ts_gdp_train, order=c(2,1,2), seasonal = list(order = c(0,0,2),period=4), include.drift = TRUE, lambda = bc_lambda) 
fit_sar2=Arima(ts_gdp_train, order=c(2,1,2), seasonal = list(order = c(0,0,3),period=4), include.drift = TRUE, lambda = bc_lambda) 
fit_sar3=Arima(ts_gdp_train, order=c(2,1,2), seasonal = list(order = c(1,0,3),period=4), include.drift = TRUE, lambda = bc_lambda) 
...
As the AICc score of (2,1,2)(0,0,3)[4] among them was the lowest, we are changing the SArima indicated SARIMA(2,1,2)(0,0,3)[4] model with drift.
We will now check forecasts from this model

## Analysing Forecasts from the Seasonal Arima Model
```{r}
forecast_sarima=forecast(fit_sar2, h= number_of_forecasts)
plot(forecast_sarima, main = "Sarima(2,1,2)(0,0,3)[4] Forecasted Values")
